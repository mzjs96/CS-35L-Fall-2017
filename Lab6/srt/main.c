...
*/ //there is no need for these lines
    ...
    int * partition = (int*) malloc(nthreads * sizeof(int));

    if (partition == NULL)
    {
        fprintf(stderr, "Partition memory allocation failed.");
        exit(1);
    }
    ...
    for (i = 0; i < nthreads; i++)
    {
        res = pthread_create(&thread[i], NULL, pixel_process, &partition[i]);
        if (res)
        {
            fprintf(stderr, "Error creating thread.");
            return -1;
        }
        partition[i+1] = partition[i] + 1;
    }
...
void * pixel_process(void * arg)
{
    ...
    /* for every pixel */
    for( int px= *par; px<width; px = px + nthreads )
    {
